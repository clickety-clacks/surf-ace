...
**Pane label** — the provider-assigned user-visible identifier for a pane (`1`, `2`, `3`, ...). `paneLabel` is distinct from `paneId` and is a secondary key for the live Surf Ace topology. Pane labels are globally unique across the provider's live/admitted surfaces. The CLU/user-facing pane token is the pane's `displayId` / `paneAddress`, which is the pane label rendered as a string; the window label is rendered and reported separately.
...
Provider authority reconciliation invariant: after a surface has accepted matching `providerId`, `sessionId`, `ownershipEpoch`, and `surfaceId`, the provider-approved identity in `authority.state` is authoritative. Valid same-provider window/pane label or pane-lineage disagreement is reconciliation work: the client MUST adopt the provider label/identity or wait for provider topology repair, and MUST NOT remain in terminal non-actionable/yellow state for that disagreement. Runtime PID/process inventory, lease/session/runtime ids, endpoint ids, display order, and presentation labels are diagnostics or transient active-session gates; they are not durable ownership authority and MUST NOT create terminal non-actionability for a same-install provider. The durable Surf Ace provider identity file is the extension-install identity authority when it is well-formed; malformed or unreadable durable identity state MUST be repaired from known local Surf Ace state or regenerated, while a well-formed conflicting durable provider identity is different-install evidence. The only terminal ownership block is evidence that the surface is owned by a different provider installation, such as mismatched stable `providerId` or foreign ownership lock without explicit takeover.
