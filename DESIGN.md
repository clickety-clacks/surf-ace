...
All controller↔surface communication uses the public WebSocket protocol. The controller is the WS client; the surface app runs the WS server. There is no REST API. OpenClaw and the OpenClaw-independent resident controller each maintain persistent connections, handle reconnect, and buffer surface state for local reads. The native Rust `surf-ace` CLI is a thin local client of the resident controller and never opens a surface WebSocket.
...
**Provider/controller capability** — a product component that uses the public Surf Ace WebSocket protocol. OpenClaw supplies one connection-holding extension. Surf Ace supplies one OpenClaw-independent resident controller that owns Bonjour discovery, persistent connections, reconnects, stable controller identity, and the durable bounded fleet projection. The native Rust `surf-ace` CLI is directly callable by any local program, script, user, or agent, but it communicates only with that controller through a local Unix socket. Tight Beam is one reusable-skill consumer of the same CLI bytes. Neither controller form is Surf Ace client state, topology, cursor, or retention authority.
...
**Local buffer** — a bounded controller-side read projection populated from client-authoritative consumable scopes. OpenClaw and the resident controller maintain it asynchronously over their background connections. The `surf-ace` CLI reads it through the resident controller's local socket. Ordinary reads never trigger surface network I/O. The projection is not cursor, pending, overflow, or unread authority.

**Connection job** — each OpenClaw or resident-controller per-surface background process that maintains the WS connection, runs the pair handshake, handles reconnect, and syncs local state. The `surf-ace` CLI has no connection job and cannot connect to a surface endpoint. Controller connection work is fully opaque to CLI callers.
...
1. **WebSocket-only surface transport.** All controller↔surface communication runs over the public WebSocket protocol. The controller is the WS client; the surface app runs the WS server. There is no REST API. OpenClaw and the OpenClaw-independent resident controller hold their sockets persistently. The `surf-ace` CLI communicates only with the resident controller through a local Unix socket.
...
### 4.9 Resident Controller, Local Surf Ace CLI, and Reusable Consumers

1. Surf Ace exposes one native Rust `surf-ace` local controller client, directly callable by any local program, script, user, or agent. Tight Beam is one separate reusable-skill consumer of the identical installed executable through ordinary command execution; it does not own or redefine the executable, crate, package, commands, controller identity, runtime, configuration, state model, authority, or access gate. The delivered tree contains no Surf Ace MCP declaration/server/tool, MCP-only adapter, dedicated Surf Ace archetype, Tight-Beam-specific binary, direct-surface CLI route, or parallel fallback route. Attaching the Tight Beam skill changes neither the archetype identity nor unrelated archetype material.
2. The supported CLI surface is exactly `list`, `push`, `read`, `topology-intent`, `topology-realize`, `clear`, `annotations-remove`, `capture-pane`, `surface-intent`, `target-register`, and `target-apply`. Inputs, acknowledgements, errors, and results are deterministic JSON. The controller socket, state root, controller product label, and per-operation friendly chat label are external inputs; no machine, role, address, surface, topology, or provenance label is compiled into the CLI or controller.
3. One resident-controller state root atomically retains the stable controller instance ID, discovered endpoints, durable `surfaces.list` then `panes.list` fleet topology, bounded projection, sticky gaps, projected cursors, acknowledgement outbox, resume metadata, and unresolved request/receipt correlations. Discovery loss marks an endpoint unavailable without deleting its durable topology. A stable advertised fingerprint, or the Bonjour instance name when no fingerprint exists, keeps endpoint identity stable across address changes.
4. The resident controller owns Bonjour discovery and every client WebSocket. It pairs or resumes with the durable ID, reconciles client-ordered snapshots/deltas/gaps, flushes acknowledgements, resolves uncertain requests, persists resulting state, and automatically reconnects. Each short-lived CLI invocation opens only the local Unix socket, sends one versioned request, receives one correlated response, and disconnects. No CLI surface WebSocket or parallel controller route exists.
5. A controller mutation remains connected until its exact correlated `operationReceipt` is durably stored and returned. Interruption after send and before durable receipt returns deterministic `outcome_unknown`, never success or an automatic retry. The resident controller must resolve all such IDs before another mutation. `target.apply` is the one asynchronous materialization seam: after pure validation and capacity checks, the client atomically persists the exact `intent_committed` response, receipt, and surface-charged work item before handing the response to the transport; browser/native materialization begins only after that send attempt. The receipt proves committed intent, never materialization success.
...
7. Pair/resume is a barrier after the prior connection's mutation-seam work. `operation.receipt.sync` yields exactly `resolved_success`, `resolved_failure`, `not_committed`, `still_pending`, or `receipt_unavailable` with `controller_reclaimed` cause. Stored success/failure is replayed exactly without request re-execution; `not_committed` alone permits a later new request ID; `still_pending` keeps the controller read/sync-only; unavailability is legal only after deterministic whole-controller-bundle reclamation. For `target.apply`, stored success replays the exact committed-intent response and does not infer materialization. The resident controller persists replay before `operation.receipt.ack`; the client retains an accepted receipt until the controller durably records that acceptance, and an idempotent release then deletes it. A crash on either side of the accepted-ack/local-marker boundary therefore replays the same terminal instead of converting it to `not_committed`. This cross-connection resolution is separate from connection-scoped request replay.
...
9. The same Rust local-client source, all eleven commands, local protocol correlation, deterministic fixtures, and Linux package manifest must pass without caller-specific carve-outs. Direct non-Tight-Beam invocation proves the local CLI product boundary. Separately, the Tight Beam skill must invoke that exact installed executable path/digest from two ordinary current archetypes and a future-archetype fixture without identity changes.
...
Returned frames are removed from the local projection transaction, the projected cursor advances, and an acknowledgement intent is durably appended before return. The client cursor advances only after acknowledgement acceptance; OpenClaw or the resident controller delivers it in the background.
...
8. OpenClaw's or the resident controller's background sender transmits acknowledgements. Reconnect reconciles the durable outbox, client cursor/gap generations, and missing projection ranges idempotently.
...
OpenClaw and the OpenClaw-independent resident controller maintain persistent WS connections to discovered screens automatically. CLU never initiates, manages, or tears down those connections. The `surf-ace` CLI has no surface connection job or MCP gate: for every caller, including Tight Beam, each short-lived invocation validates one command and sends it to the resident controller through the local Unix socket.
...
**Reads** are always local projection transactions. The client owns authoritative bounded consumable scopes, cursor floors, pending truth, and structured gaps. Pair/resume snapshots plus ordered deltas received during OpenClaw's or the resident controller's connection job keep the controller projection current. An ordinary read returns cached data/loss/status, durably advances the projected cursor and queues an idempotent acknowledgement intent, and never triggers synchronous surface network I/O. Only client acceptance of the acknowledgement advances authoritative state.
...
In one durable local transaction, returned frames/registers are consumed from the projection, projected cursor/live dirty markers advance, local alert presentation resets, and an idempotent acknowledgement intent enters the durable outbox. Client truth changes only after client acknowledgement acceptance; OpenClaw or the resident controller delivers it in the background.
