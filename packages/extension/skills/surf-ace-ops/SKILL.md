...
- `surf_ace_realize_topologies`: applies multi-surface pane topology mutations and authorized top-level Surf Ace app window lifecycle operations (`openWindow`, `closeWindow`) across supported app hosts.
...
6. For top-level app window lifecycle, use `surf_ace_realize_topologies` with `openWindow` or `closeWindow`; use pane topology tools only for panes inside an existing surface window.
