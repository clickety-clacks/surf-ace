  ...
  private async handleMessage(
    ...
    switch (message.op) {
        ...
        socket.send(
          JSON.stringify(
            this.response(message.id, "pair.request", {
              ...
              resumed: typeof message.payload?.resume === "object" && message.payload?.resume !== null,
      ...
      case "content.apply": {
        ...
        const nextRevision = Number(message.payload?.revision ?? pane.revision + 1);
        const idempotentReplay = pane.contentId === nextContentId && pane.revision === nextRevision;
...
test("surf ace runtime enforces spec-aligned provider behavior", async (t) => {
  ...
  await t.test("migration-seeded pane labels advance the allocator before later splits", async () => {
    await withRuntimeHarness(async ({ runtime, server }) => {
      ...
      assert.deepEqual(server.topologyApplyRequests.at(-1)?.paneIds, [server.initialRemotePaneId, 42]);
  ...
  await t.test("annotation alert gate is surface-scoped and resets on read or timeout", async () => {
    ...
    await withRuntimeHarness({
      ...
      run: async ({ alertBodies, runtime, server }) => {
        ...
        const secondRemotePaneId = server.topologyApplyRequests.at(-1)?.paneIds[1];
