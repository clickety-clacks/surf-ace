...
test("surf ace runtime enforces spec-aligned provider behavior", async (t) => {
  ...
  await t.test("passive persisted screen repair keeps pane labels surface-local", async () => {
    const port = nextPort++;
    const server = new FakeSurfAceWsServer(port);
  ...
  await t.test("listScreens exposes only the CLU surface fields and local pane identities", async () => {
    await withRuntimeHarness(async ({ runtime, server }) => {
      ...
      assert.equal(screen.panes[0]?.displayId, "a1");
      assert.equal(screen.panes[0]?.paneAddress, "a1");
  ...
  await t.test("content push reconciles visible pane identity before reporting applied", async () => {
    await withRuntimeHarness(async ({ runtime, server }) => {
      ...
      assert.equal(pushed.displayId, "a6");
      assert.equal(pushed.paneAddress, "a6");
      ...
      assert.equal(listedPane.displayId, "a6");
      assert.equal(listedPane.paneAddress, "a6");
      ...
      assert.equal(read.displayId, "a6");
      assert.equal(read.paneAddress, "a6");
  ...
  await t.test("multiple surfaces expose unique window labels and surface-local pane labels", async () => {
    ...
    try {
      ...
      assert.deepEqual(displayIds.sort(), ["a1", "b1"]);
      assert.deepEqual(visibleAddresses.sort(), ["a1", "b1"]);
  ...
  await t.test("pane label repair does not rewrite matching labels on different windows", async () => {
    const portA = nextPort++;
    const portB = nextPort++;
    const serverA = new FakeSurfAceWsServer(portA, { surfaceId: "sf_surface-a" });
    const serverB = new FakeSurfAceWsServer(portB, { surfaceId: "sf_surface-b" });
    ...
    try {
      ...
      assert.deepEqual(
        ...
        [
          ...
          { paneLabel: 1, windowLabel: "b" },
        ],
      );
      assert.deepEqual(screens.flatMap((screen) => screen.panes.map((pane) => pane.displayId)).sort(), ["a1", "b1"]);
      assert.deepEqual(screens.flatMap((screen) => screen.panes.map((pane) => pane.paneAddress)).sort(), ["a1", "b1"]);
  ...
  await t.test("provider pane labels remain surface-local before exposure", async () => {
    const portA = nextPort++;
    const portB = nextPort++;
    ...
    try {
      ...
      const paneLabels = screens.flatMap((screen) => screen.panes.map((pane) => pane.paneLabel));
      assert.equal(paneLabels.every((paneLabel) => Number.isInteger(paneLabel) && paneLabel > 0), true);
      assert.deepEqual(paneLabels, [1, 1]);
      assert.equal(new Set(visibleCoordinates).size, visibleCoordinates.length);
      assert.deepEqual(displayIds.sort(), ["fw1", "fx1"]);
      assert.deepEqual(visibleAddresses.sort(), ["fw1", "fx1"]);
      assert.deepEqual(
        [serverA.pairRequests[0]?.initialPaneLabel, serverB.pairRequests[0]?.initialPaneLabel].sort(),
        [1, 1],
      );
      ...
      assert.equal(repairedState.nextPaneLabel, 6244);
    } finally {
      await runtime.stop();
      await serverB.close();
      await serverA.close();
  ...
  await t.test("duplicate persisted window labels are repaired before visible coordinate exposure", async () => {
    ...
    try {
      ...
      const paneLabels = screens.flatMap((screen) => screen.panes.map((pane) => pane.paneLabel));
      assert.deepEqual(paneLabels.sort((left, right) => left - right), [1, 1]);
      assert.equal(new Set(visibleCoordinates).size, visibleCoordinates.length);
      assert.deepEqual(
        [serverA.pairRequests[0]?.windowLabel, serverB.pairRequests[0]?.windowLabel].sort(),
        ["a", "b"],
  ...
  await t.test("provider splits panes with provider-assigned pane ids and closes them by pane scope", async () => {
    await withRuntimeHarness(async ({ runtime, server }) => {
      ...
      assert.deepEqual(close, { displayId: "a2", ok: true, paneAddress: "a2", paneId: splitPaneIds[1], paneLabel: 2 });
  ...
  await t.test("provider reconciles client-listed orphan panes into topology authority before close", async () => {
    await withRuntimeHarness(async ({ runtime, server }) => {
      ...
      assert.deepEqual(close, { displayId: "a5", ok: true, paneAddress: "a5", paneId: orphanPane.paneId, paneLabel: 5 });
      assert.deepEqual(server.topologyApplyRequests.at(-1), {
  ...
  await t.test("read and snapshot remain local projections without provider reconciliation", async () => {
    await withRuntimeHarness(async ({ runtime, server }) => {
      const before = (await runtime.listScreens())[0]!;
      const firstPane = before.panes[0]!;
  ...
  await t.test("surf_ace_list projects pane label repair without mutating or publishing topology", async () => {
    await withRuntimeHarness(async ({ runtime, server }) => {
      const firstPaneId = await livePaneId(runtime, server.surfaceId, 1);
      await runtime.split({
        count: 2,
      ...
      assert.equal(panes[1].paneLabel, Number(panes[1].remotePaneId));
      await new Promise((resolve) => {
        setTimeout(resolve, 50);
      });
  ...
  await t.test("provider captures frame-open state freshly, delivers settled annotation turns, and ignores after_reconnect snapshot hints", async () => {
    await withRuntimeHarness(async ({ alertBodies, annotationTurns, runtime, server, warnings }) => {
      ...
      assert.match(turn.message, /Surf Ace settled annotation on surface "Surface A", pane a1\./);
      ...
      assert.match(turn.message, /"displayId": "a1"/);
      assert.match(turn.message, /"paneAddress": "a1"/);
  ...
  await t.test("remote-paired foreign rows with cached panes stay hidden without local ownership", async () => {
    await withRuntimeHarness(async ({ discovery, runtime, server, stateDir }) => {
      ...
      assert.equal(server.topologyApplyRequests.length, topologyRequestsBeforeFreshPair);
