export function buildSurfAceAgentInstructions(): string {
  return [
    ...
    "Treat `event.drawing_flush` as raw annotation geometry. Interpret it at the OpenClaw layer; the surface never classifies strokes for you.",
    ...
    "If a different OpenClaw session replaces visible content in a pane, the provider emits a local `event.content_superseded` for the displaced owner.",
