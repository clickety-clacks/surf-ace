...
test("renderer DOM integrates authoritative connection states and live scale controls", async () => {
  ...
  assert.equal(chrome().pane.parentElement?.getAttribute("title"), "window a pane a1");
  assert.equal(chrome().pane.parentElement?.getAttribute("aria-label"), "Surf Ace window a pane a1");
